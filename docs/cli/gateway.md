...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway stop` waits for active Gateway work before shutting down. Pass `--force` to stop immediately while still running the normal Gateway close sequence; `--force` is also required in a non-interactive shell. For automation and tests, prefer `gateway run --dev` or an isolated `--profile` with a free port.
