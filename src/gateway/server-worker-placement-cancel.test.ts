...
it("Stop waits for canonical cancelled terminal persistence and rejects input arriving during that write", async () => {
  ...
  const events: Array<{ phase: unknown; status: unknown; aborted: unknown; stopReason: unknown }> =
    [];
  ...
  try {
    ...
    if (fresh.ok) {
      fresh.value.cleanupAdmittedRun({ force: true });
    ...
    await reclaim?.catch(() => {});
    if (active?.ok) {
      active.value.cleanupAdmittedRun({ force: true });
